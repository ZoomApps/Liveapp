...
Define("PhotoGallery",
    ...
    function () {
        ...
        this.New = function () {
            ...
            var batch = new Batch();
            Application.Camera.TakePhoto(
                function (img) {

                    if (newrec.Keys && newrec.Keys.length > 0)
                        newrec.Keys[0].split(',').forEach(function (key) {
                            newrec[key] = null;
                        });

                    newrec.Temp = false;
                    newrec[m_form.Fields[0].Name] = img;
                    batch.Insert(newrec);
                },
                Application.OptionValue(m_form.Options, "quality"),
                Application.OptionValue(m_form.Options, "maxwidth"),
                function () {
        ...
        function AddImage(rec_) {
            ...
            var li = $('<li data-sub-html="' + (m_form.Fields.length > 1 ? Default(rec[m_form.Fields[1].Name], '') : '') + '" data-src="data:image/jpeg;base64,' + photo + '" style="padding-left: 0; list-style: none; display: inline-block; padding: -3px; height: 190px; vertical-align: top;">' +
                ...
                (m_form.Fields.length > 1 ? '<div id="desc' + id + '" class="photo-desc" style="padding: 4px;box-sizing: border-box;max-width: 120px;text-overflow: ellipsis;white-space: nowrap;overflow-x: hidden;overflow-y: hidden;">' + Default(rec[m_form.Fields[1].Name], '') + '</div>' : '') +
                '</li>');
