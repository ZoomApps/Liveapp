	...
	Croppic.prototype = {
		...
		crop:function(){
			...
			var cropData = {
					...
					imgW:that.imgW,
					imgH:that.imgH,
					imgY1: $('.cropImgWrapper').offset().top - that.imgEyecandy.offset().top,
					imgX1: $('.cropImgWrapper').offset().left - that.imgEyecandy.offset().left,
					cropH:that.objH,
					cropW:that.objW,
            ...
            if(that.options.cropUrl == ''){
                if (that.options.onAfterImgCrop) that.options.onAfterImgCrop.call(that, cropData);
                that.destroy();
                return;
            }
